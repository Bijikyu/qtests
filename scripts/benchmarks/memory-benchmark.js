 ...
 * 
 * Tests memory pressure detection and adaptive scaling under various load conditions
 */

// Use CommonJS for compatibility
const { MemoryPressureMonitor } = require('../../lib/memoryPressure.js');
const { PerformanceMonitor } = require('../../lib/performanceMonitor.js');

class MemoryScalabilityBenchmark {
  constructor() {
    ...
    this.results = [];
    console.log('Memory Scalability Benchmark Initialized');
  }

  async runAllBenchmarks() {
    console.log('Starting comprehensive memory scalability benchmarks...');
    ...
    const scenarios = [
      {
        ...
        duration: 30000,
        expectedActions: ['minimal_adaptive_scaling']
      },
      {
        name: 'Moderate Load', 
        targetMemory: 300 * 1024 * 1024,
        duration: 30000,
        expectedActions: ['cache_size_reduction', 'moderate_scaling']
      },
      {
        name: 'High Load',
        targetMemory: 600 * 1024 * 1024,
        duration: 30000,
        expectedActions: ['aggressive_scaling', 'forced_gc', 'cache_reduction']
      },
      {
        name: 'Extreme Load',
        targetMemory: 900 * 1024 * 1024,
        duration: 20000,
        expectedActions: ['emergency_scaling', 'massive_cleanup', 'request_rejection']
      }
    ];
    ...
    for (const scenario of scenarios) {
      ...
      console.log(`Completed: ${scenario.name} - Memory: ${Math.round(result.peakMemory / 1024 / 1024)}MB, Pressure: ${Math.round(result.memoryPressure * 100)}%`);
      
      await this.recoveryPeriod(5000);
    }
  ...
  async runScenario(scenario) {
    ...
    try {
      ...
      while (Date.now() < endTime) {
        ...
        if (memoryStats) {
          maxMemoryPressure = Math.max(maxMemoryPressure, memoryStats.percentage);
        }
        
        const recentActions = this.memoryMonitor.getScalingHistory(5000);
        if (recentActions.length > actionCount) {
          recentActions.slice(actionCount).forEach(action => {
            scalingActions.push({
      ...
      return {
        ...
        peakMemory,
        memoryPressure: maxMemoryPressure,
        adaptiveScalingActions: scalingActions,
        cleanupPerformance: cleanupTime,
        memoryFreed: Math.max(0, startMemory.heapUsed - endMemory.heapUsed),
  ...
  generateReport() {
    console.log('\nMEMORY SCALABILITY BENCHMARK REPORT');
    console.log('='.repeat(50));
    
    for (const result of this.results) {
      const status = result.success ? 'PASS' : 'FAIL';
      const peakMemoryMB = Math.round(result.peakMemory / 1024 / 1024);
      const avgActions = result.adaptiveScalingActions.length / (result.totalDuration / 60000);
      ...
      console.log(`  Max Pressure:    ${Math.round(result.memoryPressure * 100)}%`);
      console.log(`  Scaling Actions:  ${result.adaptiveScalingActions.length}`);
      console.log(`  Actions/Minute:   ${avgActions.toFixed(1)}`);
      console.log(`  Cleanup Time:    ${result.cleanupPerformance}ms`);
      console.log(`  Memory Freed:    ${Math.round(result.memoryFreed / 1024)}KB`);
    ...
    const avgPressure = successful.reduce((sum, r) => sum + r.memoryPressure, 0) / successful.length;
    ...
    console.log(`  Avg Pressure:     ${Math.round(avgPressure * 100)}%`);
    ...
    if (avgPressure > 0.8) {
      ...
      console.log('   → Increase cleanup frequency');
    }
    
    if (avgCleanupTime > 5000) {
      console.log('SLOW CLEANUP PERFORMANCE');
      console.log('   → Optimize cleanup algorithms');
      console.log('   → Consider incremental cleanup');
    }
...
if (require.main === module) {
  const benchmark = new MemoryScalabilityBenchmark();
