...
function demonstrateSecurityPolicies() {
    ...
    const validation = securityMonitor.validateSecurityHeaders ?
        securityMonitor.validateSecurityHeaders(headers) :
        { valid: true, errors: [] };
...
function demonstrateRateLimiting() {
    ...
    userIdentifiers.forEach((userId, index) => {
        ...
        if (!result.allowed) {
            ...
            console.log(`  Retry After: ${result.retryAfter ? result.retryAfter + 'ms' : 'N/A'}`);
        }
    });
}
/**
...
export { demonstrateBasicValidation, demonstrateSecurityMonitoring, demonstrateSecurityPolicies, demonstrateRateLimiting, demonstrateSecurityReport };
