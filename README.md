...
Notes:
- On real runs (no `--dry-run`), the generator writes `jest.config.js`, `setup.ts`, creates `qtests-runner.ts`, and updates `package.json` test script to `npx tsx qtests-runner.ts`.
