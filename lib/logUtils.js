...
function logStart(functionName, ...args) {
  const argsString = args.map(arg => safeSerialize(arg)).join(`, `); // create readable args string with backticks
  if (LOG_ENABLED) console.log(`[START] ${functionName}(${argsString})`); // log start with backticks
}
...
function logReturn(functionName, returnValue) {
  const valueString = safeSerialize(returnValue); // serialize return value safely
  if (LOG_ENABLED) console.log(`[RETURN] ${functionName} -> ${valueString}`); // log return with backticks
}
...
function executeWithLogs(name, fn, ...args) {
  ...
  try {
    const argString = args.length ? args.map(a => safeSerialize(a)).join(`, `) : `none`; // serialize args with backticks
    if (LOG_ENABLED) console.log(`${name} is running with ${argString}`); // start log with backticks
    
    const result = fn(...args); // invoke callback function
    
    if (result && typeof result.then === `function`) { // handle promise result with backticks
      return result.then(res => { // await async resolution
        if (LOG_ENABLED) console.log(`${name} is returning ${safeSerialize(res)}`); // async return log
        console.log(`executeWithLogs is returning ${res}`); // logging return value per requirements
        return res;
      }).catch(err => { // async error handling
        if (LOG_ENABLED) console.log(`${name} encountered ${err.message}`); // async error log
        console.log(`executeWithLogs error: ${err.message}`); // error logging per requirements
        throw err;
      });
    }
    
    if (LOG_ENABLED) console.log(`${name} is returning ${safeSerialize(result)}`); // sync return log
    console.log(`executeWithLogs is returning ${result}`); // logging return value per requirements
    return result;
  } catch (error) {
    if (LOG_ENABLED) console.log(`${name} encountered ${error.message}`); // sync error log
    console.log(`executeWithLogs error: ${error.message}`); // error logging per requirements
    throw error;
  }
}
