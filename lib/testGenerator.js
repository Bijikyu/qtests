...
class TestGenerator {
  ...
  extractExports(content) {
    ...
    const esExports = [...content.matchAll(PATTERNS.exportsES)];
    esExports.forEach(match => {
      if (match[1]) exports.add(match[1]);
    });
    
    // Extract CommonJS exports
    const commonJSExports = [...content.matchAll(PATTERNS.exportsCommonJS)];
    ...
    const functionDecls = [...content.matchAll(PATTERNS.functionDeclarations)];
    const classDecls = [...content.matchAll(PATTERNS.classDeclarations)];
    
    // Check if functions/classes are mentioned in exports
    functionDecls.forEach(match => {
      if (match[1] && (content.includes(`exports.${match[1]}`) || content.includes(`module.exports.${match[1]}`) || content.includes(`module.exports = ${match[1]}`))) {
        exports.add(match[1]);
      }
    });
