...
`.trim();
  ...
  generateTestRunner() {
    ...
    // Read the existing qtests-runner.cjs as template
    const templatePath = path.join(__dirname, '..', 'qtests-runner.cjs');
    let runnerContent;
    ...
    if (fs.existsSync(templatePath)) {
      ...
      runnerContent = `// qtests Test Runner - Auto-generated by qtests (CommonJS)
// This file discovers and runs all tests in your project
// Uses .cjs extension for ES module project compatibility
...
`;
    ...
    const created = this.writeIfMissing('qtests-runner.cjs', runnerContent);
  ...
  updatePackageJsonTestScript() {
    try {
      ...
      packageJson.scripts.test = 'node qtests-runner.cjs';
      
      fs.writeFileSync(packagePath, JSON.stringify(packageJson, null, 2), 'utf8');
      console.log('✅ Updated package.json test script to use qtests-runner.cjs');
      return true;
    } catch (error) {
