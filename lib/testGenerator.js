...
class TestGenerator {
  constructor(options = {}) {
    ...
    // Auto-discover source directory if not explicitly set and default doesn't exist
    if (!options.SRC_DIR && !fs.existsSync(this.config.SRC_DIR)) {
      this.config.SRC_DIR = this.discoverSourceDirectory();
    }
  }

  /**
   * Intelligently discover source directory by looking for common patterns
   */
  discoverSourceDirectory() {
    const commonSourceDirs = ['src', 'lib', 'app', 'source', 'code'];
    const currentDir = process.cwd();
    
    // First, try common source directory names
    for (const dir of commonSourceDirs) {
      const fullPath = path.join(currentDir, dir);
      if (fs.existsSync(fullPath) && this.hasSourceFiles(fullPath)) {
        console.log(`📁 Auto-discovered source directory: ${dir}`);
        return dir;
      }
    }
    
    // If no common directories found, look for directories with source files
    const entries = fs.readdirSync(currentDir, { withFileTypes: true });
    const candidates = entries
      .filter(entry => entry.isDirectory())
      .filter(entry => !this.shouldSkipDirectory(entry.name))
      .map(entry => entry.name)
      .filter(dirName => this.hasSourceFiles(path.join(currentDir, dirName)));
    
    if (candidates.length > 0) {
      // Prefer directories with more source files
      const bestCandidate = candidates.reduce((best, current) => {
        const bestCount = this.countSourceFiles(path.join(currentDir, best));
        const currentCount = this.countSourceFiles(path.join(currentDir, current));
        return currentCount > bestCount ? current : best;
      });
      console.log(`📁 Auto-discovered source directory: ${bestCandidate}`);
      return bestCandidate;
    }
    
    // Fallback to current directory if no dedicated source directory found
    console.log(`📁 No dedicated source directory found, scanning current directory`);
    return '.';
  }
  ...
  shouldSkipDirectory(dirName) {
    const skipPatterns = [
      ...
      'tests', 'test', '__tests__', 'spec', '.vscode', '.idea',
      'docs', 'documentation', 'assets', 'public', 'static'
    ];
  ...
  hasSourceFiles(dir) {
    ...
    try {
      ...
      return files.some(file => this.config.VALID_EXTS.includes(path.extname(file)));
    } catch (error) {
      return false;
    }
  }
  ...
  /**
   * Count source files in directory (for ranking candidates)
   */
  countSourceFiles(dir) {
    ...
    try {
      const files = this.walk(dir);
      return files.filter(file => this.config.VALID_EXTS.includes(path.extname(file))).length;
    } catch (error) {
      return 0;
    }
  }
