...
class TestGenerator {
  ...
  getRelativeTestPath(file, type = 'unit') {
    ...
    if (type === 'unit') {
      // For unit tests, place them alongside the source file
      return path.join(dir, `${basename}.test${ext}`);
    } else {
      // For API/integration tests, use the configured test directory
      const rel = path.relative(process.cwd(), file);
      return path.join(this.config.TEST_DIR, rel.replace(/\.[tj]sx?$/, `.test.js`).replace(/[\\/]/g, '__'));
    }
  }
  ...
  createApiTest(method, route) {
    ...
    lines.push(`    // Create mock app for testing`);
