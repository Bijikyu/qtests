...
`.trim();
  ...
  /**
   * Generate qtests test runner file - Full-featured TypeScript ES module
   */
  generateQtestsRunner(): void {
    try {
      // Copy the existing full-featured runner from bin/qtests-ts-runner as template
      const binRunnerPath = path.join(process.cwd(), 'bin', 'qtests-ts-runner');
      
      if (fs.existsSync(binRunnerPath)) {
        // Read the full-featured runner from bin directory
        const binRunnerContent = fs.readFileSync(binRunnerPath, 'utf8');
        
        // Modify the shebang and header for generated version
        const generatedRunnerContent = binRunnerContent
          .replace('#!/usr/bin/env node', '#!/usr/bin/env tsx')
          .replace('// IMPORTANT: This CLI is sacrosanct and not generated. Do not overwrite.', '// Generated by qtests - Full-featured CLI tool for running tests')
          .replace('// Provides batching, summaries, and debug report creation for failing tests.', '// This file is auto-generated by qtests but safe to customize')
          .replace('// qtests Test Runner - Feature-rich CLI with batching, summaries, and debug file', '// qtests Test Runner - Full-featured CLI tool for running tests')
          .replace('// Restores original functionality while ensuring Jest is launched with project config', '// Generated by qtests - provides parallel execution, batching, and debug features');
        
        // Write the generated runner
        fs.writeFileSync('qtests-ts-runner.ts', generatedRunnerContent, 'utf8');
        
        console.log('✅ Generated qtests-ts-runner.ts with full features (parallel batching, DEBUG_TESTS.md)');
      } else {
        console.warn('⚠️  bin/qtests-ts-runner not found, cannot generate runner');
      }
