...
async function main() {
  ...
  if (ordered.length === 0) {
    if (!silent) console.log(`${colors.yellow}⚠  No test files found${colors.reset}`);
    if (!isEnvTruthy('QTESTS_NO_RESULTS_FILE')) {
      const resultsPath = (process.env.QTESTS_RESULTS_FILE && String(process.env.QTESTS_RESULTS_FILE).trim())
        || path.join(projectRoot, 'qtests-results.json');
      try {
  ...
  if (!silent) {
    for (const r of perFileResults) {
      const indicator = r.success ? `${colors.green}${colors.bold}✓${colors.reset}` : `${colors.red}${colors.bold}✗${colors.reset}`;
    ...
    console.log(`${colors.green}✓ Passed:${colors.reset} ${colors.bold}${passedFiles}${colors.reset}`);
    console.log(`${colors.red}✗ Failed:${colors.reset} ${colors.bold}${failedFiles}${colors.reset}`);
    console.log(`${colors.bold}═══════════════════════════════════════${colors.reset}`);
  }

  await generateDebugFile(perFileResults.filter((r) => !r.success));

  const writeResults = !isEnvTruthy('QTESTS_NO_RESULTS_FILE');
  if (writeResults) {
